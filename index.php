...
{
    global $root;
    $root = 'C:/Users/usuario/Desktop/Magento-Module-Creator-master/Magento-Module-Creator-master/';
    if (!is_array($from)) {
        $from = array($from);
    }
    ...
    /*
        Hasta esta instancia el root es: $root = C
        Por lo que se debe hardcodear la ruta del $root para que tome el directorio correcto
    */
        
    $root = 'C:/Users/usuario/Desktop/Magento-Module-Creator-master/Magento-Module-Creator-master/';
    for ($i = 0; $i < count($to); $i++) {        
        if (copy($root.$from[$i], $shop.$to[$i]) === false) {
            throw new Exception('Could not copy blank files.');
        }
    }
    return true;
}
